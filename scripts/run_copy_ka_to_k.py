...
Copy Devanagari Letter Ka from Noto Sans Devanagari to 'k' position in hindixv38
"""
...
def main():
    ...
    # Step 2: Open hindixv38 and paste to k
    print("Step 2: Pasting into hindixv38 at 'k' position...")
    print("-" * 70)
    try:
        hindixv38 = fontforge.open(str(sfd_path))
        
        k_unicode = 0x006B  # k (Latin Small Letter K)
        if k_unicode not in hindixv38:
            print(f"✗ Glyph k (U+{k_unicode:04X}) not found")
            hindixv38.close()
            sys.exit(1)
        
        glyph = hindixv38[k_unicode]
        ...
        hindixv38.selection.select(k_unicode)
        hindixv38.paste()
